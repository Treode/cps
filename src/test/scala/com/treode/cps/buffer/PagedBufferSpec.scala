...
class PagedBufferSpec extends FlatSpec {
  ...
  it should "set and get a byte" in {
    ...
    for (i <- 0 until m) (b.setByte (i, i.toByte))
    expectResult ((0, 0, n)) (b.readAt, b.writeAt, b.capacity)
    b.writeAt = m
    for (i <- 0 until m) expectResult (i.toByte) (b.getByte (i))
  ...
  it should "write and read a byte" in {
    ...
    for (i <- 0 until m) (b.writeByte (i.toByte))
    expectResult ((0, m, n)) (b.readAt, b.writeAt, b.capacity)
    for (i <- 0 until m) expectResult (i.toByte) (b.readByte ())
  ...
  it should "discard once" in {
    def checkDiscard (m2: Int, m3: Int) {
      ...
      for (i <- m2 until m) expectResult (i.toByte) (b.getByte (i-m2))
      for (i <- m3 until m) expectResult (i.toByte) (b.readByte ())
  ...
  it should "discard twice" in {
    def checkDiscard (m1: Int, m2: Int) {
      ...
      for (i <- m1 until m) expectResult (i.toByte) (b.getByte (i-m1))
      val m2 = 2345
      b.readAt = m2
      b.discard (m2)
      expectResult ((0, m-m1-m2, n-m1-m2)) (b.readAt, b.writeAt, b.capacity)
      for (i <- m1+m2 until m) expectResult (i.toByte) (b.getByte (i-m1-m2))
    }
  ...
  it should "slice and dice" in {
    def checkSlice (discard: Int, npages: Int) = {
      ...
      for (i <- 0 until m3) (b.writeByte (i.toByte))
      b.readAt = discard
      b.discard (discard)
      val b2 = b.slice (m1-discard, m2-m1)
      expectResult ((0, m2-m1)) (b2.readAt, b2.writeAt)
      for (i <- m1 until m2) expectResult (i.toByte) (b2.readByte ())
    }
  ...
  it should "set and get byte arrays" in {
    def checkSetBytes (npages: Int) = {
      ...
      for (i <- 0 until m2-m1) (bytes (i) = i.toByte)
      b.setBytes (m1, bytes, 0, bytes.length)

      for (i <- 0 until 123) expectResult (i.toByte) (b.readByte ())
      for (i <- 0 until m2-m1) expectResult (i.toByte) (b.readByte ())
      for (i <- m2 until m3) expectResult (i.toByte) (b.readByte ())
    }
    ...
    def checkGetBytes (npages: Int) = {
      ...
      for (i <- m1 until m2) expectResult (i.toByte) (bytes (i-m1))
    }
  ...
  it should "set and get byte arrays after discarding" in {
    def checkSetBytes (npages: Int) = {
      ...
      for (i <- m0 until m1) expectResult (i.toByte) (b.readByte ())
      for (i <- 0 until m2-m1) expectResult (i.toByte) (b.readByte ())
      for (i <- m2 until m3) expectResult (i.toByte) (b.readByte ())
    }
